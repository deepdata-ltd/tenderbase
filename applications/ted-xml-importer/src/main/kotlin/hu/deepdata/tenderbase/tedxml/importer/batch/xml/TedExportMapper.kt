 ...
 */
class TedExportMapper(val converter: IConverter<String, TedExport>) : RecordProcessor<Record<XmlRecord>, Record<XmlRecord>> {

	companion object : KLogging()

	override fun processRecord(record: Record<XmlRecord>) = record.apply {
		logger.trace("Parsing XML: ${record.payload.xmlMeta.name}")
		payload!!.tedExport = converter.convert(payload!!.xmlContent)
	}
}