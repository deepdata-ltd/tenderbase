 ...
 */
class XmlMetaFetcher(private val fileMetaRepo: FileMetaRepo) :
	...
	override fun processRecord(record: Record<XmlRecord>): Record<XmlRecord> {
		logger.trace("Fetching XML metadata: ${record.payload.xmlMeta.name}")
		with(record.payload) {
			val id = xmlMeta.id!!
			xmlMeta = fileMetaRepo.findById(id) ?: xmlMeta
		}
		return record
	}
}