...
public class Milestone {
	 ...
	 */
	@JsonProperty("id")
	@JsonPropertyDescription("A local identifier for this milestone, unique within this block. This field is used to keep track of multiple revisions of a milestone through the compilation from release to record mechanism.")
	private String id;
	/**
	 * Title
	 * <p>
	 ...
	 * Milestone type
	 * <p>
	 * The type of milestone, drawn from an extended [milestoneType codelist](http://standard.open-contracting.org/latest/en/schema/codelists/#milestone-type).
	 */
	@JsonProperty("type")
	@JsonPropertyDescription("The type of milestone, drawn from an extended [milestoneType codelist](http://standard.open-contracting.org/latest/en/schema/codelists/#milestone-type).")
	private String type;
	/**
	 * Description
	 * <p>
	 ...
	 */
	@JsonProperty("code")
	@JsonPropertyDescription("Milestone codes can be used to track specific events that take place for a particular kind of contracting process. For example, a code of 'approvalLetter' could be used to allow applications to understand this milestone represents the date an approvalLetter is due or signed. Milestone codes is an open codelist, and codes should be agreed among data producers and the applications using that data.")
	private String code;
	/**
	 * Due date
	 * <p>
	...
	@JsonPropertyDescription("The date the milestone was last reviewed or modified and the status was altered or confirmed to still be correct.")
	 ...
	 */
	@JsonProperty("status")
	@JsonPropertyDescription("The status that was realized on the date provided in dateModified, drawn from the [milestoneStatus codelist](http://standard.open-contracting.org/latest/en/schema/codelists/#milestone-status).")
	...
	public String toString() {
		return new ToStringBuilder(this).append("id", id).append("title", title).append("type", type).append("description", description).append("code", code).append("dueDate", dueDate).append("dateMet", dateMet).append("dateModified", dateModified).append("status", status).append("documents", documents).toString();
	}

	@Override
	public int hashCode() {
		return new HashCodeBuilder().append(code).append(documents).append(dueDate).append(dateMet).append(description).append(dateModified).append(id).append(title).append(type).append(status).toHashCode();
	}
	...
	public boolean equals(Object other) {
		...
		return new EqualsBuilder().append(code, rhs.code).append(documents, rhs.documents).append(dueDate, rhs.dueDate).append(dateMet, rhs.dateMet).append(description, rhs.description).append(dateModified, rhs.dateModified).append(id, rhs.id).append(title, rhs.title).append(type, rhs.type).append(status, rhs.status).isEquals();
	}
