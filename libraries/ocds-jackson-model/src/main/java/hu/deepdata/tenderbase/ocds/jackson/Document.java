...
public class Document {
	 ...
	 */
	@JsonProperty("id")
	@JsonPropertyDescription("A local, unique identifier for this document. This field is used to keep track of multiple revisions of a document through the compilation from release to record mechanism.")
	private String id;
	/**
	 * Document type
	 * <p>
	...
	@JsonPropertyDescription("A classification of the document described taken from the [documentType codelist](http://standard.open-contracting.org/latest/en/schema/codelists/#document-type). Values from the provided codelist should be used wherever possible, though extended values can be provided if the codelist does not have a relevant code.")
	...
	@JsonPropertyDescription("A short description of the document. We recommend descriptions do not exceed 250 words. In the event the document is not accessible online, the description field can be used to describe arrangements for obtaining a copy of the document.")
	 ...
	 */
	@JsonProperty("url")
	@JsonPropertyDescription(" direct link to the document or attachment. The server providing access to this document should be configured to correctly report the document mime type.")
	private URI url;
	/**
	 * Date published
	 * <p>
	...
	@JsonPropertyDescription("The date on which the document was first published. This is particularly important for legally important documents such as notices of a tender.")
	...
	public String toString() {
		return new ToStringBuilder(this).append("id", id).append("documentType", documentType).append("title", title).append("description", description).append("url", url).append("datePublished", datePublished).append("dateModified", dateModified).append("format", format).append("language", language).toString();
	}

	@Override
	public int hashCode() {
		return new HashCodeBuilder().append(datePublished).append(documentType).append(format).append(description).append(dateModified).append(language).append(id).append(title).append(url).toHashCode();
	}
	...
	public boolean equals(Object other) {
		...
		return new EqualsBuilder().append(datePublished, rhs.datePublished).append(documentType, rhs.documentType).append(format, rhs.format).append(description, rhs.description).append(dateModified, rhs.dateModified).append(language, rhs.language).append(id, rhs.id).append(title, rhs.title).append(url, rhs.url).isEquals();
	}
